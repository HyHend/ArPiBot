...
{
  if(angle < 0) { angle = 0; }   // Bugfix for sensor min location
  if(angle > 180) { angle = 180; } // Bugfix for sensor max location
...
{ 
  ...
  if((previousSensorAngleLoopMillis + sensorAngleLoopMillis) < currentMillis) {
    ...
    if(currentMillis > ultrasonicSensorServoMovingEnd
      ...
      ultrasonicSensorServoMovingEnd = currentMillis + wait;
    }
  }
  ...
  if(ultrasonicMeasureStart > 20
    ...
    if(ultrasonicMeasureDistance < 1000) {
